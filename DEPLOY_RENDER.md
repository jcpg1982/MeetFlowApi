...
Este documento contiene las instrucciones para levantar el servidor tanto de forma local (para pruebas) como en producción (Render).
...
Crea un archivo llamado `.env` dentro de la carpeta `API` y pega lo siguiente (ajusta los valores según tu base de datos local):
```env
...
---
**IMPORTANTE**: El archivo `.gitignore` ya está configurado para que no subas tus contraseñas por error. No borres las entradas `.env` ni `*.json` del gitignore.
