...
- Are focused on a single change (i.e. avoid large refactoring or style adjustments in untouched
  code if not the primary goal of the pull request)
...
The `npm test -- dev` implements watching for tests within Node and `karma start` may be used for
